...
class MMD_loss(nn.Module):
    ...
    def conditional_mmd(self, source, target, s_label, t_label):
    ...
    def marginal_mmd(self, source, target):
    ...
    def get_loss2(self, source, target, s_label, t_label):
        batch_size = source.size()[0]
        weight_ss, weight_tt, weight_st = self.tcal_weight(
            s_label, t_label, batch_size=batch_size, class_num=self.class_num)
    ...
    def cal_weight(self, s_label, t_label, batch_size=32, class_num=31):
        ...
        s_vec_label = self.convert_to_onehot(s_sca_label, class_num)
    ...
    def tconvert_to_onehot(self, label, batch_size, class_num):
        return torch.zeros(batch_size, class_num).cuda().scatter_(1, label, 1)

    def tcal_weight(self, s_label, t_label, batch_size=512, class_num=2):
        batch_size = s_label.size()[0]
        s_sca_label = s_label
        s_vec_label = self.tconvert_to_onehot(s_sca_label.view(-1, 1), batch_size, class_num)
